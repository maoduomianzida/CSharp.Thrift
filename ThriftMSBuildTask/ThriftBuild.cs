...
namespace ThriftMSBuildTask
{
    ...
    public class ThriftBuild : Task
    {
        ...
        /// </summary>
        [Required]
        public ITaskItem ThriftGeneratorParam { get; set; }
        ...
        /// 判断是否需要重新编译
        /// </summary>
        /// <param name="lastBuildPath">上次编译的缓存文件地址</param>
        /// <param name="lastWrite">IDL文件夹中的最近一次修改时间</param>
        /// <returns></returns>
        private bool NeedRecompile(string lastBuildPath, string lastWrite)
        {
        ...
        /// 利用thrift.exe生成 代码
        /// </summary>
        /// <param name="thriftFileArr">thrift文件集合</param>
        /// <returns>生成成功或者失败</returns>
        private bool GenerateCode(string thriftDir, string[] thriftFileArr)
        {
            bool result = true;
            //run the thrift executable to generate C#
            foreach (string thriftFile in thriftFileArr)
            {
                ...
                p.StartInfo.Arguments = $"--gen csharp{ThriftGeneratorParam} -o " + SafePath(thriftDir) + " -r " + thriftFile;
        ...
        /// 编译自动生成的thrift code
        /// </summary>
        /// <param name="thriftDir">thrift文件夹目录</param>
        /// <param name="outputPath">编译完成后的输出文件</param>
        /// <returns>编译是否成功</returns>
        private bool CompileThriftCode(string thriftDir, string outputPath)
        {
            Csc csc = new Csc();
            ...
            LogMessage("正在编译自动生成的 thrift code...", MessageImportance.Normal);
        ...
        public override bool Execute()
        {
            ...
            if (!NeedRecompile(lastBuildPath, lastWrite))
                return true;

            //find the directory of the thriftlibrary (that's where output will go)
            FileInfo thriftLibInfo = new FileInfo(ThriftLibrary.ItemSpec);
            ...
            if (thriftFileArr.Length > 0)
            {
                ...
                {
                    LogMessage("编译自动生成的 thrift code失败！", MessageImportance.High);
